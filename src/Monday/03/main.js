...
const checkCharacterAge = characters => {
    return characters.map(el => el.age === 'Unknown' ? "Возраст неизвестен" : el.age)
};
...
const checkCharacterFriends = characters => {
    // return characters.filter(el => !el.friends.length && el)
    // return characters.filter(el=>el.friends.length === 0 ? el : '')
    return characters.filter(el => !el.friends.length).map(el => el.name)

    // return characters.reduce((acc, el)=>{
    //     if(!el.friends.length){
    //         acc.push(el.name)
    //     }
    //     return acc
    // }, [])
    ...
    // return characters.reduce((acc, el) => {
    //     !el.friends.length ? acc.push(el.name) : ''
    //     return acc
    // }, [])
};
...
const checkCharacterIsHobbit = characters => {
    return characters.filter(el => el.race !== 'Hobbit' ? el : '').map(el => el.name)
    // return characters.filter(el => el.race !== 'Hobbit' && el.name).map(el=>el.name)
    // return characters.reduce((acc, el) => {
    //     el.race !== 'Hobbit' ? acc.push(el.name) : ''
    //     return acc
    // }, [])
};
...
const checkCharacterIdIsEven = characters => {
    // return characters.filter(el => el.id % 2 === 0 && el).map(el=>el.name)
    return characters.reduce((acc, el) => {
        if (el.id % 2 === 0) {
            return [...acc, el.name]
        }
        return acc
        // el.id % 2 === 0 ? acc.push(el.name) : ''
        //     return acc
    }, [])
};
...
const checkMaxFriends = characters => {
    ...
    return [...characters].sort((a, b) => a.friends.length - b.friends.length)[characters.length - 1].name
};
